...
Additionally, the DDPM authors introduce the usage of self-attention <d-cite key="vaswani2017attention"></d-cite> at the 16x16 resolution, for both downsampling and upsampling. Because the original dot-product attention operation is computationally expensive, we opt to use a linear self-attention variant from <d-cite key="shen2021efficient"></d-cite> in downsampling/upsampling layers, which can be written as:

[TODO: latex for linear self-attention operation]

```python
class LinearAttention(nn.Module):
    """Linear dot product attention variant: https://arxiv.org/abs/1812.01243"""
...
For the bottleneck layer of the U-Net, the DDPM authors used a self-attention layer sandwiched by two ResNet blocks. In our implementation, we use the original dot-product self-attention operation at this part of the model, which can be expressed as

[TODO: Latex for original dot-product self-attention]
...
Thus, the DDPM U-Net architecture (for the CIFAR-10 dataset, with image resolution (32, 32, 3)) is as follows:
...
[TODO: code block for the full U-Net implementation]

As discussed above, this implementation has several differences from the original U-Net architecture:
...
3. We use a weight-standardized Conv2d layer, which is thought to work well with group normalization.

These changes are adopted from subsequent work which seeks to improve the U-Net architecture, such as <d-cite key="dhariwal2021diffusion"></d-cite>.
